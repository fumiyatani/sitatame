...
type Expectation struct {
	...
	// view after this step. Checked against the *currently visible
	// terminal screen* — the runner replays the cumulative teatest byte
	// stream through a vt10x virtual terminal sized to WindowSize and
	// matches against the resulting cell grid. A match means the
	// substring is on screen now, not just that it appeared in some
	// earlier frame.
	ViewContains []string
	// ViewNotContains is the negative form: each substring must be absent
	// from the *currently visible terminal screen* after this step. Useful
	// for confirming that a guard hid an element (e.g. "RANGE" tag
	// disappears after Tab) or that an off-screen item is no longer
	// rendered after scrolling. Asserting against the reconstructed screen
	// (not the cumulative byte history) is what makes this assertion
	// meaningful — bubbletea's output stream is append-only, so a "not
	// contains" check over the whole stream could never pass once the
	// forbidden substring had been drawn once.
	...
	// against the reconstructed terminal screen for this step.
	ViewGolden string
}
